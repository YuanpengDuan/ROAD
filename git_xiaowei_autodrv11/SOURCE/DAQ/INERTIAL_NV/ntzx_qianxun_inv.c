...
{   
    ...
    if (  g_gps_data.qual == NTZX_QIANXUN_QUAL_RTKFIX   )
    {
        ...
        nav_info->lat = g_gps_data.lat;//原始数据经纬度是实际经纬度的100倍，详细可用串口助手查看GPS原始数据
...
static int ntzx_qianxun_usart_init(void)
{
    ...
    qianxun_usart_info.tty_dev_name = "/dev/ttyUSB1"; // 驱动加载的文件名“/dev/tty....”
    // qianxun_usart_info.tty_dev_name = "/dev/ttyS1"; 
